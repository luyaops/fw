...
func WithFields(fields Fields) *log.Entry {
	return std.WithFields(log.Fields(fields))
}

// Debug logs a message at level Debug on the standard logger.
func Debug(args ...interface{}) {
	std.WithField("position", caller()).Debug(args...)
}
...
func Error(args ...interface{}) {
	std.WithField("position", caller()).Error(args...)
}
...
func Debugf(format string, args ...interface{}) {
	std.WithField("position", caller()).Debugf(format, args...)
}
	...
	if len(gopaths) >= 1 && gopaths[0] != "" {
		for _, path := range gopaths {
			if strings.HasPrefix(file, path) {
				file = strings.TrimPrefix(file, filepath.Join(path, "src")+"/")
...
func IsDebug() bool {
	return std.Level == log.DebugLevel
}
